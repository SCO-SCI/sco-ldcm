...
# NOTE (Phase 3 merge, Step 3): resolver cache loading and the refresh
# scheduler are NOT started here. The parent app (parent_app.py) loads the
# shared NEA/ExoFOP caches once and runs a single refresh scheduler. Both
# sub-apps import the resolvers from `shared`, so their /api/resolve and
# health routes read the one shared cache populated by the parent.
...
# NOTE (Phase 3 merge, Step 3): the refresh scheduler thread is NOT started
# here. The parent app runs a single shared scheduler. The _refresh_scheduler
# and _attempt_refresh_both_caches helpers above are retained but unused by the
# sub-app; the parent owns refresh.
...
def compute(
    ...
    filter: str = Query(..., alias="filter", description="Filter code (e.g. 'V', 'G', 'Kp', 'TESS', 'C')"),
...
<title>sco-ldc4p cache status</title>
...
@app.get("/")
def root():
    # Phase 3 merge, Step 4: the merged frontend is served by the parent at "/".
    # This sub-app owns only /api/* ; its root just points to the main page.
    return JSONResponse(
        content={"service": "four-parameter sub-app", "api_base": "/fourparam/api",
                 "frontend": "/"},
    )
