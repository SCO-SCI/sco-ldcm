...
# Filter registry
#
# Four-parameter ("4-term" non-linear) limb-darkening coefficients a1..a4,
# Claret Eq. 5 (= Eq. 3 in Claret & Southworth 2023):
#     I(mu)/I(1) = 1 - a1(1-mu^0.5) - a2(1-mu) - a3(1-mu^1.5) - a4(1-mu^2)
#
# Two catalogue sources supply the coefficients:
#   CB2011 -> Claret & Bloemen (2011, A&A 529, A75), tableeq5.dat.
#             ATLAS (all filters) and PHOENIX (plane-parallel). Long format,
#             multi-metallicity, filter/method/model in-row.
#   CS2023 -> Claret & Southworth (2023, A&A 674, A63), table4/8/12 (Eq.3, M2).
#             PHOENIX-COND spherical, solar metallicity only. Wide format.
#
# Model/source partition (Option B):
#   * ATLAS coefficients come exclusively from CB2011.
#   * PHOENIX-COND coefficients come from CS2023 for every band it covers
#     (Gaia, Kepler, TESS, CHEOPS, SDSS, Stromgren, Johnson-Cousins incl. JHK).
#   * CoRoT and Spitzer are absent from CS2023, so their PHOENIX rows are taken
#     from CB2011's (older, 2011 plane-parallel) PHOENIX data to maximise
#     coverage. These two are the only bands whose PHOENIX-regime data is the
#     2011 computation; they display as plain "PHOENIX", cited as CB2011.
#
# Source tags used below:
#   CB_CS   -> ATLAS from CB2011, PHOENIX-COND from CS2023
#   CS_ONLY -> PHOENIX-COND from CS2023 only (Gaia, TESS, CHEOPS: post-2011)
#   CB_ONLY -> CB2011 only: ATLAS, plus (for CoRoT/Spitzer) 2011 PHOENIX
#
# Registry naming/categories follow the QUADRATIC engine (JHK = "Johnson J/H/K"
# under Johnson-Cousins, Spitzer split into S1..S4, etc.), NOT the power-2
# engine's 2MASS labelling.
# ---------------------------------------------------------------------------
_CB_CS   = "CB_CS"
_CS_ONLY = "CS_ONLY"
_CB_ONLY = "CB_ONLY"
...
# CS2023 PHOENIX-COND grids store under "PHOENIX" but display as "PHOENIX-COND".
# CoRoT (C) and Spitzer (S1..S4) get their PHOENIX from CB2011 (2011 plane-
# parallel) -- they are deliberately absent here, so they display as plain
# "PHOENIX", which is physically correct.
_CS23_COND_FILTERS = [
    "U", "B", "V", "R", "I", "J", "H", "K",
    "u*", "g*", "r*", "i*", "z*",
    "u", "v", "b", "y",
...
# ---------------------------------------------------------------------------
# Grid storage. Coefficients are the four-parameter tuple (a1, a2, a3, a4).
# The interpolation machinery is identical to the quadratic/power-2 systems;
# only the coefficient arity (2 -> 4) differs.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# CB2011 parser (long format, one row per node+filter).
#
# tableeq5.dat byte layout (CB2011 ReadMe):
#    1- 5  logg     7-12  Teff    14-17  Z     19-22  xi
#   24-30  a1      32-38  a2     40-46  a3    48-54  a4
#   56-57  Filt      59  Met(L/F)   61-67  Mod(ATLAS/PHOENIX)
#
# Option B keep-rule: Met == 'L' and xi == 2.0 and
#   ( Mod == ATLAS ) OR ( Mod == PHOENIX and Filt in {C,S1,S2,S3,S4} ).
# The source tag stored per row follows the registry: CoRoT/Spitzer -> CB_ONLY;
# every other CB2011 filter -> CB_CS (its PHOENIX-COND partner comes from CS2023,
# so only its ATLAS rows are ingested here).
# ---------------------------------------------------------------------------

# In tableeq5.dat the SDSS bands carry a trailing prime that CDS renders as a
# comma in the 2-byte code field ("u," "g," "r," "i," "z,"). Map these to the
# registry's canonical SDSS codes ("u*" etc.). Strömgren u/v/b/y and Johnson
# U..K are space-padded and unaffected.
_CB_SDSS_CODE_MAP = {
    "u,": "u*", "g,": "g*", "r,": "r*", "i,": "i*", "z,": "z*",
}

# Option B: keep CB2011 PHOENIX only for the bands CS2023 does not cover, i.e.
# CoRoT and Spitzer. (In practice CB2011 tableeq5 contains PHOENIX rows for
# CoRoT but NOT for Spitzer -- Spitzer is ATLAS-only in this catalogue -- so
# CoRoT ends up the sole filter with 2011-provenance PHOENIX data. Listing
# S1..S4 here is harmless and future-proof.)
...
# ---------------------------------------------------------------------------
# CS2023 parser (wide format, one row per (logg, Teff) node; solar only).
#
# Token layout (whitespace-tokenised, mirrors power-2 _parse_cs23 but with FOUR
# coefficient blocks instead of two):
#     logg Teff Z xi | a1[0..n-1] | a2[0..n-1] | a3[0..n-1] | a4[0..n-1]
#                    | mu_cri[0..n-1] | chi2[0..n-1]
# Total tokens = 4 + 6*nbands.  mu_cri and chi2 are read past, not stored.
# Every CS2023 band is PHOENIX-COND -> stored under model "PHOENIX".
# `bands` is the ordered (filter_code, source_tag) list for the file.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Source-file layout under data/:
#   data/c11/tableeq5.dat            CB2011 four-parameter (ATLAS + PHOENIX)
#   data/c23/table4.dat              CS2023 Gaia/Kepler/TESS/CHEOPS (Eq.3, M2)
#   data/c23/table8.dat              CS2023 SDSS ugriz         (Eq.3, M2)
#   data/c23/table12.dat             CS2023 Stromgren+Johnson-Cousins (Eq.3, M2)
# ---------------------------------------------------------------------------
