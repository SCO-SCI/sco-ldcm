...
def maxted_values(a1: float, a2: float, a3: float, a4: float,
                  mu_cri: Optional[float] = None) -> Dict[str, object]:
    """Maxted's h1' and h2' from the four-parameter coefficients.

    I2 is the brightness at the second position.  It has no name in Maxted's
    scheme and exists only to be subtracted.
    """
    mu1, mu2, k = _maxted_positions(mu_cri)
    coeffs = (a1, a2, a3, a4)
    h1 = 1.0 - sum(c * (1.0 - mu1 ** ((j + 1) / 2.0))
                   for j, c in enumerate(coeffs))
    I2 = 1.0 - sum(c * (1.0 - mu2 ** ((j + 1) / 2.0))
                   for j, c in enumerate(coeffs))
    return {"h1_prime": h1, "h2_prime": h1 - I2,
            "edge_point_applied": mu_cri is not None,
            "mu_cri": mu_cri,
            "realizable": _realizable(h1, h1 - I2)}
