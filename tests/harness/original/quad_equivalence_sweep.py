...
def enumerate_queries(tables, nodes_only=False):
    """Yield (filter_code, model, teff, logg, feh, kind) in a DETERMINISTIC order.

    kind is 'node' or 'center'. Order: sorted keys, then all nodes (sorted),
    then all cell centers (sorted). Deterministic so the checkpoint index is
    stable across runs.
    """
    for key in sorted(tables.keys()):
        # key is (source, filter, model); the API takes filter + model
        filt = key[1]
        model = key[-1]
        g = tables[key]
        data = g["data"]
        ...
        for t in te:
            for l in lg:
                for z in fe:
                    if (t, l, z) in nodeset:
                        yield (filt, model, t, l, z, "node")

        if nodes_only:
            continue

        # full cell centers (all 8 corners present)
        for i in range(len(te) - 1):
            for j in range(len(lg) - 1):
                for k in range(len(fe) - 1):
                    ...
                    if all(c in nodeset for c in corners):
                        ct = round((te[i] + te[i + 1]) / 2.0, 4)
                        cl = round((lg[j] + lg[j + 1]) / 2.0, 4)
                        cz = round((fe[k] + fe[k + 1]) / 2.0, 4)
                        yield (filt, model, ct, cl, cz, "center")


def count_queries(tables, nodes_only=False):
    return sum(1 for _ in enumerate_queries(tables, nodes_only))
...
def main():
    ...
    ap.add_argument("--checkpoint", default="quad_sweep_checkpoint.json")
    ap.add_argument("--out", default="quad_sweep_discrepancies.csv")
    ...
    total = count_queries(tables, args.nodes_only)
    print(f"  total queries: {total:,}  ({'nodes only' if args.nodes_only else 'nodes + cell centers'})")
    ...
    probe = next(enumerate_queries(tables, args.nodes_only))
    f, m, t, l, z, _ = probe
    ...
    try:
        for q in enumerate_queries(tables, args.nodes_only):
